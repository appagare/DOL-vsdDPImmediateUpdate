...
Public Class vsdDPImmediateUpdate
    ...
    Private Const vsdDPImmediateUpdateversion As String = "v200512131333"
    ...
    Friend Enum SelectSPStatus
        SelectSPFailed = 0
        SelectSPSuccessful = 1
    End Enum
...
#Region " Component Designer generated code "
    ...
    Shared Sub Main()
        ...
        '
        ServicesToRun = New System.ServiceProcess.ServiceBase() {New vsdDPImmediateUpdate}
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        '
        'vsdDPImmediateUpdate
        '
        Me.ServiceName = "vsdDPImmediateUpdate"
...
#Region "Service Code"
    ...
    End Function
    ''' <summary>
    '''     Returns the content of the <clientid></clientid> element from the complete MSMQ message.
    ''' </summary>
    ''' <param name="Message">Entire MSMQ message.</param>
    ''' <remarks>
    '''     The MSMQ message is an XML string. 
    '''     This function parses the XML to obtain the contents of the <clientid> element. 
    '''     Should an error occur, such as the XML fail to load and/or parse because of bad input, returns a null string. 
    ''' </remarks>
    Private Function GetClientId(ByVal Message As String) As String

        Dim XML As New Xml.XmlDocument
        XML.LoadXml(Message)
        Return XML.SelectSingleNode("qmsg/clientid").FirstChild.Value

    End Function
    ' <summary>
    '''     Returns the status value of the stored procedure, based on the code.
    ''' </summary>
    ''' <param name="ResponseCode">Response code that was parsed from the stored procedure call.</param>
    ''' <remarks>
    ...
    Private Sub LogEvent(ByVal Source As String, _
        ...
        'log message
        LogEventObject.LogEvent(Me.ServiceName, Source & " (" & vsdDPImmediateUpdateversion & ")", Message, MessageType, LogType)
        ...
        If ForceEmail = True Or (MessageType = MessageType.Error AndAlso Now >= ConfigValues.LastEmailSent.AddSeconds(ConfigValues.EmailFrequency)) Then
            ...
            'send the e-mail
            LogEventObject.LogEvent(Me.ServiceName, Source & " (" & vsdDPImmediateUpdateversion & ")", Message, MessageType, LogType.Email)
    ...
    Private Sub ProcessMessage(ByVal State As Object)
        ...
        Dim datProcessStart As Date = Now 'used for performance testing (DebugMode > 0)
        Dim datProcessEnd As Date = Now 'used for performance testing (DebugMode > 0)
        ...
        Dim strRequest As String = "" 'stored procedure request in string form
        Dim dsRequest As DataSet = Nothing 'stored procedure request
        Dim strResponse As String = "" 'stored procedure response
        Dim bytIdx As Byte = 0 'index for multiple stored procedure calls
        ...
        Try
            ...
            strTranType = Trim(Left(strBuffer, 2)) 'get the tran type from the buffer
            ...
            For bytIdx = 0 To CByte(TranTypeUtil.CallCount - 1)
                ...
                If TranTypeUtil.IsProcessCallCompleted(XML, bytIdx) = False Then
                    ...
                    dsRequest = TranTypeUtil.GetRequest(strBuffer, bytIdx, bytOnlineUpdateIndicator, ExceptionAction)

                    'extra debugging
                    If ConfigValues.DebugMode > 1 Then
                        Me.LogEvent("ProcessMessage", "Debug Request: [" & DataSetRowToString(dsRequest) & "]", MessageType.Debug, LogType.Standard)
                    End If

                    If dsRequest.Tables.Count = 0 Or bytOnlineUpdateIndicator > bytIdx Then
                        'Test Case #7
                        'the only time TranTypeUtil.GetRequest should return 
                        'an empty dataset w/out throwing an error or if the OnlineUpdateIndicator is greater than 
                    ...
                    'clear this before each stored procedure call
                    CallStatus = ResponseStatus.NotSet

                    datProcessStart = Now 'capture the start time of the stored procedure call
                    If ConfigValues.DebugMode > 1 Then
                        Me.LogEvent("ProcessMessage", "Debug Request: [" & DataSetRowToString(dsRequest) & "]", MessageType.Information, LogType.Standard, True)
                    End If

                    strResponse = Execute_Stored_Procedure(TranTypeUtil.TargetConnectStringKey(bytIdx), _
                    dsRequest)
                    'capture the end-time of the stored procedure call
                    ...
                    strMessage = TranTypeUtil.SetProcessCallStatus(XML, strResponse)
                    ...
                    TranTypeUtil.SetClientID(XML, strResponse)
            ...
            For bytIdx = 0 To CByte(TranTypeUtil.CallCount - 1)
                ...
                Try
                    ...
                    'stored procedure calls, such as 0 if none are successful (but we wouldn't be here if that happens), 
                    '1 if there is one successful call, 2 if there are two successful calls, etc.)
                    'It is also the same as bytIdx + 1
                    TranTypeUtil.MessageComplete(strBuffer, bytIdx, strResponse)
                Catch ex As Exception
                    'Test Case #11
        ...
        Catch ex As Exception
            ...
            End If
            If Not dsRequest Is Nothing Then
                strErrMessage &= " Request[" & DataSetRowToString(dsRequest) & "]"
            End If
            If strResponse <> "" Then
        ...
        If ConfigValues.DebugMode > 0 Then
            ...
            If datProcessStart > datStart AndAlso datProcessEnd > datProcessStart Then
                'get the stored procedure call processing time if we captured it
                RunLength = datProcessEnd.Subtract(datProcessStart)
                MillisecsWS = CType(RunLength.TotalMilliseconds, Integer)
                strLogMessage &= " Stored Procedure Dur: " & MillisecsWS.ToString & " ms"
            End If
    ...
    End Sub
    Protected Function Execute_Stored_Procedure(ByVal constrSP As String, _
    ByVal dsSPInput As DataSet) As String
        'Set up call to Stored Procedure based on the columns returned from selection
        Dim cmdParameters() As SqlClient.SqlParameter = Nothing
        Dim c As New DataColumn
        Dim intIdx As New Integer
        Dim UpdSPName As String = " "
        Dim SelSPStatus As SelectSPStatus = SelectSPStatus.SelectSPFailed
        Dim SelSPErrorMsg As String = " "
        Dim dsUpdResults As DataSet
        Dim strResults As String = " "

        ReDim cmdParameters(0)
        intIdx = 0
        UpdSPName = ""
        For Each c In dsSPInput.Tables(0).Columns

            If Left(CType(c.ColumnName, String), 1) = "@" Then

                If intIdx > 0 Then
                    'create a slot in our collection if necessary
                    ReDim Preserve cmdParameters(UBound(cmdParameters) + 1)
                End If
                cmdParameters(intIdx) = New SqlClient.SqlParameter
                cmdParameters(intIdx).ParameterName = c.ColumnName
                cmdParameters(intIdx).Value = dsSPInput.Tables(0).Rows(0)(c.ColumnName)
                intIdx += 1
            End If

            If CType(c.ColumnName, String) = "UpdSPName" Then
                UpdSPName = CType(dsSPInput.Tables(0).Rows(0)(c.ColumnName), String)
            End If

            If CType(c.ColumnName, String) = "SPStatus" Then
                SelSPStatus = CType(dsSPInput.Tables(0).Rows(0)(c.ColumnName), SelectSPStatus)
            End If

            If CType(c.ColumnName, String) = "ErrorMsg" Then
                SelSPErrorMsg = CType(dsSPInput.Tables(0).Rows(0)(c.ColumnName), String)
            End If

        Next

        If SelSPStatus = SelectSPStatus.SelectSPSuccessful Then

            Try
                dsUpdResults = DataObject.ExecuteDataset(constrSP, _
                                                        CommandType.StoredProcedure, _
                                                        UpdSPName, _
                                                        cmdParameters)

                If ConfigValues.DebugMode > 1 Then
                    Me.LogEvent("ProcessMessage", "Result of Requested SP: [" & DataSetRowToString(dsUpdResults) & "]", MessageType.Information, LogType.Standard, True)
                End If

                strResults = CType(dsUpdResults.Tables(0).Rows(0)(dsSPInput.Tables(0).Columns.Item(0)), String)

            Catch ex As Exception
                strResults = "Error: Stored Procedure Failed. " & ex.Message
            End Try

        Else
            strResults = "Error: Stored Procedure Error. " & SelSPErrorMsg
        End If

        Return strResults
    End Function
    Protected Function DataSetRowToString(ByVal dsToString As DataSet) As String
        Dim sbDataset As New System.Text.StringBuilder
        Dim c As DataColumn = Nothing
        Dim columnname As String = " "
        Dim columnvalue As String = " "

        If dsToString.Tables.Count = 1 AndAlso dsToString.Tables(0).Rows.Count = 1 Then
            'concatenate all fields into a string.
            For Each c In dsToString.Tables(0).Columns
                columnname = CType(c.ColumnName, String)
                If IsDBNull(dsToString.Tables(0).Rows(0)(c.ColumnName)) Then
                    columnvalue = "Null"
                Else
                    columnvalue = CType(dsToString.Tables(0).Rows(0)(c.ColumnName), String)
                End If
                sbDataset.Append(columnname & "=" & columnvalue & ", ")
            Next
            Return Left(sbDataset.ToString, (sbDataset.Length - 2))
        Else
            Return ""
        End If
    ...
    End Sub
    ''' <summary>
    '''     Connect to the vsdDPImmediateUpdate database to obtain the operating parameters.
    ...
    Private Sub ReadDBSettings()
        ...
        Do While DBConnectOK = False
            ...
            If Err.Number = 0 Then
                ...
                'get the tran code to stored procedure calls cross-reference
                ConfigValues.TranTypes = DataObject.ExecuteDataset(ConfigValues.ConnectionKey, CommandType.StoredProcedure, _
                    "selDPTranTypes", New SqlClient.SqlParameter("@strProcess", ConfigValues.ProcessMode)).Tables(0)
...
Friend Class ConfigValues
    ...
    Private _ConnectionKey As String = "" 'vsdDPImmediateUpdate connection string key
    ...
    Private _ProxyName As String = "" 'optional proxy name for the stored procedure calls
    ...
    ''' <summary>
    '''     This property sets/returns the vsdDPImmediateUpdate connection string key.
    ''' </summary>
    ...
    End Property
    ''' <summary>
    '''     This property sets/returns the optional proxy name value to use when making the stored procedure calls.
    ''' </summary>
